...
| 08-15 | **the last decile is not a representation problem** — the ceiling decomposed onto the same frames | the ceiling is FLAT across the shot: 0.000086 to 0.000120 per decile against a mean of 0.000100, and the last decile's basis share is **10.1%**, the LOWEST of the ten, against 21–33% in the middle | **A22 survives the test that would have killed it.** The tail is not harder to represent and it is not EFIT label noise — it is 27.2% of everything reachable sitting in 10% of the frames, and the most winnable region in the fold |
| 08-15 | **what the sequence model actually buys, by region** — C1 run for `mlp` alone against the ensemble | the MLP loses 0.00603 of geometry cost, the ensemble 0.00490, so the recurrence saves **0.00113 — 18.3% overall, 24.9% of the first decile and 15.2% of the last** | it helps least exactly where its physical argument is strongest. A bidirectional pass sees the termination coming and does not use it, which sharpens B5's outstanding screen: whatever the recurrence learned, it is not the ramp-down dynamics |
